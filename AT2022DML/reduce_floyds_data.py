...
def extract_floyds(
    light_fits, flat_fits, arc_fits, coeff_red=None, coeff_blue=None
):
    ...
    red.get_rectification(upsample_factor=5, coeff=coeff_red, display=False)
    ...
    red.ap_extract(model="gauss", apwidth=10, spec_id=0, display=False)
    ...
    blue.get_rectification(upsample_factor=5, coeff=coeff_blue, display=False)
    blue.apply_rectification()

    blue.ap_trace(
        nspec=1, percentile=10, trace_width=20, fit_deg=3, display=False
    )
    blue.ap_extract(model="gauss", apwidth=10, display=False)
    ...
    flat.ap_extract(model="gauss", apwidth=10, skywidth=0, display=False)
...
def calibrate_red(science, standard, standard_name):
    ...
    red_onedspec.find_arc_lines(
        display=False, prominence=0.25, top_n_peaks=25, stype="science"
    )
    red_onedspec.find_arc_lines(
        display=False, prominence=0.25, top_n_peaks=25, stype="standard"
    )
    ...
    red_onedspec.set_ransac_properties(
        minimum_matches=19, stype="science+standard"
    )
    red_onedspec.do_hough_transform(stype="science+standard")

    # Solve for the pixel-to-wavelength solution
    red_onedspec.fit(max_tries=200, stype="science+standard", display=False)

    # Apply the wavelength calibration and display it
    red_onedspec.apply_wavelength_calibration(
        wave_start=5000, wave_end=11000, wave_bin=1, stype="science+standard"
    )

    red_onedspec.load_standard(standard_name, library="esohststan")
    red_onedspec.get_sensitivity(smooth=True, method="polynomial", sens_deg=13)
    ...
    literature_flux_resampled = spectres(
        red_onedspec.standard_spectrum_list[0].wave_resampled,
        red_onedspec.standard_spectrum_list[0].wave_literature,
        red_onedspec.standard_spectrum_list[0].flux_literature,
    )
    literature_flux = spectres(
        red_onedspec.standard_spectrum_list[0].wave,
        red_onedspec.standard_spectrum_list[0].wave_literature,
        red_onedspec.standard_spectrum_list[0].flux_literature,
    )

    second_pass_correction_resampled = (
        red_onedspec.standard_spectrum_list[0].flux_resampled
        / literature_flux_resampled
    )
    second_pass_correction = (
        red_onedspec.standard_spectrum_list[0].flux / literature_flux
    )
    second_pass_correction = medfilt(second_pass_correction, 21)
    second_pass_correction_resampled = medfilt(
        second_pass_correction_resampled, 21
    )

    red_onedspec.standard_spectrum_list[0].flux /= second_pass_correction
    red_onedspec.standard_spectrum_list[0].flux_err /= second_pass_correction
    red_onedspec.standard_spectrum_list[0].flux_sky /= second_pass_correction

    red_onedspec.standard_spectrum_list[
        0
    ].flux_resampled /= second_pass_correction_resampled
    red_onedspec.standard_spectrum_list[
        0
    ].flux_err_resampled /= second_pass_correction_resampled
    red_onedspec.standard_spectrum_list[
        0
    ].flux_sky_resampled /= second_pass_correction_resampled

    red_onedspec.science_spectrum_list[0].flux /= second_pass_correction
    red_onedspec.science_spectrum_list[0].flux_err /= second_pass_correction
    red_onedspec.science_spectrum_list[0].flux_sky /= second_pass_correction
...
def calibrate_blue(science, standard, standard_name):
    ...
    blue.find_arc_lines(
        prominence=0.25, top_n_peaks=10, display=False, stype="science"
    )
    blue.find_arc_lines(
        prominence=0.25, top_n_peaks=10, display=False, stype="standard"
    )
    ...
    blue.set_ransac_properties(
        minimum_matches=9, filter_close=True, stype="science+standard"
    )
    blue.do_hough_transform(stype="science+standard")

    # Solve for the pixel-to-wavelength solution
    blue.fit(max_tries=200, stype="science+standard", display=False)

    # Apply the wavelength calibration and display it
    blue.apply_wavelength_calibration(
        wave_start=3200, wave_end=5600, wave_bin=1, stype="science+standard"
    )

    blue.load_standard(standard_name, library="esohststan")
    blue.get_sensitivity(smooth=True, method="polynomial", sens_deg=21)
    blue.inspect_sensitivity()
    blue.apply_flux_calibration()

    # second pass to correct for the small scale detector response
    literature_flux_resampled = spectres(
        blue.standard_spectrum_list[0].wave_resampled,
        blue.standard_spectrum_list[0].wave_literature,
        blue.standard_spectrum_list[0].flux_literature,
    )
    second_pass_correction = (
        blue.standard_spectrum_list[0].flux_resampled
        / literature_flux_resampled
    )
    second_pass_correction = medfilt(second_pass_correction, 15)

    blue.standard_spectrum_list[0].flux_resampled /= second_pass_correction
    blue.standard_spectrum_list[0].flux_err_resampled /= second_pass_correction
    blue.standard_spectrum_list[0].flux_sky_resampled /= second_pass_correction

    blue.science_spectrum_list[0].flux_resampled /= second_pass_correction
    blue.science_spectrum_list[0].flux_err_resampled /= second_pass_correction
    blue.science_spectrum_list[0].flux_sky_resampled /= second_pass_correction
...
filelist = np.genfromtxt(
    "filelist.list", delimiter=",", dtype="U", autostrip=True
)
...
#
standard_light_fits = fits.open(
    filename[(obstype == "standard") & (frametype == "light")][0]
)[1]
standard_flat_fits = fits.open(
    filename[(obstype == "standard") & (frametype == "flat")][0]
)[1]
standard_arc_fits = fits.open(
    filename[(obstype == "standard") & (frametype == "arc")][0]
)[1]
standard_name = "hz44"
...
# Science frame here
#
science_light_fits = fits.open(
    filename[(obstype == "science") & (frametype == "light")][0]
)[1]
science_flat_fits = fits.open(
    filename[(obstype == "science") & (frametype == "flat")][0]
)[1]
science_arc_fits = fits.open(
    filename[(obstype == "science") & (frametype == "arc")][0]
)[1]
science_name = "SN 2020 DML"
...
onedspec_blue.inspect_reduced_spectrum(
    ...
    display=False,
    save_fig=True,
    filename=filename[(obstype == "science") & (frametype == "light")][
        0
    ].split(".")[0]
    + "_blue",
)
...
onedspec_blue.inspect_reduced_spectrum(
    ...
    display=False,
    save_fig=True,
    filename=filename[(obstype == "standard") & (frametype == "light")][
        0
    ].split(".")[0]
    + "_blue",
)
...
plt.savefig(science_name.replace(" ", "_") + ".png")
...
plt.tight_layout()
plt.title(science_name)
plt.savefig(science_name.replace(" ", "_") + "_combined.png")

plt.show()

from astropy.stats import sigma_clip

telluric_factor = onedspec_red.science_spectrum_list[0].telluric_factor
telluric_profile = onedspec_red.science_spectrum_list[0].telluric_profile
wave = onedspec_red.science_spectrum_list[0].wave
flux = onedspec_red.science_spectrum_list[0].flux
flux_continuum = onedspec_red.science_spectrum_list[0].flux_continuum

plt.figure(3)
plt.clf()
plt.plot(wave, flux)
plt.plot(wave, flux_continuum)
plt.ylim(0, 1e-15)
plt.xlim(5000, 9000)

telluric_factor_resampled = onedspec_red.science_spectrum_list[
    0
].telluric_factor_resampled
telluric_profile_resampled = onedspec_red.science_spectrum_list[
    0
].telluric_profile_resampled
wave_resampled = onedspec_red.science_spectrum_list[0].wave_resampled
flux_resampled = onedspec_red.science_spectrum_list[0].flux_resampled
flux_continuum_resampled = onedspec_red.science_spectrum_list[
    0
].flux_resampled_continuum

plt.figure(4)
plt.clf()
plt.plot(wave_resampled, flux_resampled)
plt.plot(wave_resampled, flux_continuum_resampled)
plt.ylim(0, 1e-15)
plt.xlim(5000, 9000)
